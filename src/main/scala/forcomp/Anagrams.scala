...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(char => char).map(chars => (chars._1, chars._2.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match{
    case Nil => List(Nil)
    case head :: tail => {
      val s = combinations(tail)
      (for { cpt <- 1 to head._2; ss <- s}
        yield  (head._1, cpt) :: ss).toList ++ s
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences ={
   val result = for{ occ <- x} yield {
     val occ2 = y.filter(elem => elem._1.equals(occ._1))
     if (!occ2.isEmpty && occ._2 >= occ2.head._2)
       (occ._1, occ._2 - occ2.head._2)
     else occ
   }
    result.filter(elem => elem._2 != 0)
  }
